...
$list_banners = get_banners($start, $num_per_page);

#Lấy danh sách trang
$list_num_page = db_num_page('tbl_banners', $num_per_page);

#Tổng số bảng ghi lấy từ database 
$total_row = db_num_rows("SELECT * FROM `tbl_banners`");
...
$total_approved = db_num_rows("SELECT * FROM `tbl_banners` WHERE `banner_status` = 'Approved'");

#Tổng bài viết chờ xét duyệt
$total_waiting = db_num_rows("SELECT * FROM `tbl_banners` WHERE `banner_status` = 'Waiting...'");

#Tổng bài viết đã xóa
$total_trash = db_num_rows("SELECT * FROM `tbl_banners` WHERE `banner_status` = 'Trash'");
?>
<!-- Lấy phần header -->
