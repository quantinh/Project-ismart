...
$list_products_cat = db_search_products_cat_by_page($value, $start, $num_per_page);

#Tổng bài viết đã phê duyệt
$total_approved = db_num_rows("SELECT * FROM `tbl_products_cat` WHERE `cat_status` = 'Approved' AND `cat_title` LIKE '%$value%'");

#Tổng bài viết chờ xét duyệt
$total_waiting = db_num_rows("SELECT * FROM `tbl_products_cat` WHERE `cat_status` = 'Waiting...' AND `cat_title` LIKE  '%$value%'");

#Tổng bài viết đã xóa
$total_trash = db_num_rows("SELECT * FROM `tbl_products_cat` WHERE `cat_status` = 'Trash' AND `cat_title` LIKE '%$value%'");
