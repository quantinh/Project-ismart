...
function addCatAction()
{
    ...
    if (isset($_POST['btn-add-cat'])) {
        ...
        if (empty($_POST['cat_title'])) {
            ...
            if(is_exists('tbl_posts_cat', 'cat_title', $_POST['cat_title'])){
        ...
        if (empty($_POST['cat_slug'])) {
            ...
            if(is_exists('tbl_posts_cat','cat_slug', $_POST['cat_slug'])){
                $error['cat_slug'] = "(*) Link thân thiện đã tồn tại !";
            } else {
                $cat_slug = create_slug($_POST['cat_slug']);
            }
        }
        #Kiểm tra danh mục danh mục
        ...
        if (empty($error)) {
            ...
            $error['cat'] = "Thêm danh mục mới thành công !" . "<br>" . "<a href='?mod=posts&controller=postCat&action=index'>Trở về danh sách các danh mục</a>";
            // echo $add_cat;
            // show_array($data);
            // show_array($error);
        }
...
function updateCatAction() 
{
    ...
    if (isset($_POST['btn-update-cat'])) {
        ...
        if (empty($error)) {
            ...
            update_cat($data, $cat_id);
            // show_array($data);
            $error['cat'] = "Cập nhật danh mục thành công !" . "<br>" . "<a href='?mod=posts&controller=postCat&action=index'>Trở về danh sách danh mục</a>";
        }
    }
    load_view('updateCat');
}
...
function deleteCatAction()
{
    ...
    delete_cat($cat_id);
    redirect('?mod=posts&controller=postCat&action=index');
    load_view('listCat');
}
