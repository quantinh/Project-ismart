...
$list_posts_cat_all = db_search_all_posts_cat($value);

$total_row = count($list_posts_cat_all);

#Tổng số lượng bảng ghi trên 1 trang
$num_per_page = 6;

#Tổng số bảng ghi từ danh sách tất cả danh mục
$total_row = count($list_posts_cat_all);
...
$list_posts_cat = db_search_posts_cat_by_page($value, $start, $num_per_page);

#Tổng bài viết đã phê duyệt
$total_approved = db_num_rows("SELECT * FROM `tbl_posts_cat` WHERE `cat_status` = 'Approved' AND `cat_title` LIKE '%$value%'");

#Tổng bài viết chờ xét duyệt
$total_waiting = db_num_rows("SELECT * FROM `tbl_posts_cat` WHERE `cat_status` = 'Waiting...' AND `cat_title` LIKE  '%$value%'");

#Tổng bài viết đã xóa
$total_trash = db_num_rows("SELECT * FROM `tbl_posts_cat` WHERE `cat_status` = 'Trash' AND `cat_title` LIKE '%$value%'");
        ...
        <div id="content" class="fl-right">
            ...
            <div class="section" id="detail-page">
                ...
                <div class="filter-wp clearfix">
                    ...
                    <form method="POST" action="?mod=posts&controller=postCat&action=applyPost&post_id=<?php echo $page_num;?>">    
                        <div class="actions">   
                        ...
                        <div class="table-responsive">
                            ...
                            <?php } else { $error['cat'] = "(*) Không có danh mục bài viết nào !"; ?>
                                    <p class="error"><?php echo $error['cat'] ?></p>
                            <?php } ?>
                        </div>
                    </form>
                </div>
