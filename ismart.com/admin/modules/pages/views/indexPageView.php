...
#Lấy danh sách trang
$list_num_page = db_num_page('tbl_pages', $num_per_page);
...
$total_approved = db_num_rows("SELECT * FROM `tbl_pages` WHERE `page_status` = 'Approved'");

#Tổng bài viết chờ xét duyệt
$total_waiting = db_num_rows("SELECT * FROM `tbl_pages` WHERE `page_status` = 'Waiting...'");

#Tổng bài viết đã xóa
$total_trash = db_num_rows("SELECT * FROM `tbl_pages` WHERE `page_status` = 'Trash'");
?>
<!-- Lấy phần header -->
